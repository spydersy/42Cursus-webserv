 ...
 * fffff() function :***********************************************************
 */
Response fffff(Request clientReq, Servers &servers) { // FIRST INTEGRATION --- to ==> responseHandler
    std::cout << KGRN << "----------------------- RES_HANDLER -----------------------" << KNRM << std::endl;
	Response clientRes(clientReq, servers);
	std::cout << "DBG_PATH :[" << clientReq.getPath() << "]" << std::endl;
    clientRes.setHttpVersion();
    // std::cout << "DBG HTTP_VERSION : [" <<  clientRes.get_responseBuffer() << "]" << std::endl;
	clientRes.setHttpStatus();
    // std::cout << "DBG HTTP_STATUS : [" << std::endl <<  clientRes.get_responseBuffer() << "]" << std::endl;
    // std::cout << KRED << "response BUFFER : [[[" << response.get_responseBuffer() << "]]]" << KNRM << std::endl;
    // write(1, clientRes.get_responseBuffer().c_str(), clientRes.get_responseBuffer().length());
...
void	accept_connections( std::vector<Server> &servers, ClientRequest &read_clients, fd_set &backup_rset ) {
		...
		if (FD_ISSET(it->first.getClientFd(), &backup_rset)) {
			...
			if ((recvLength = recv(it->first.getClientFd(), buffer, RECV_SIZE, 0)) != -1) {
				...
				if (it->second.add_buffer(recvLength, buffer) == true) {
					...
					// Handle request
					Response clientRes = fffff(it->second, servers); // FIRST INTEGRATION --- to ==> responseHandler
					// ***************************************** YIIIIIIIIIISSS ;
					write_clients.push_back(std::make_pair(it->first, clientRes)); // SECOND INTEGRATION --- from(`it->first, it->second`) ===> to(`clientRes->first, clientRes->second`)
					ClientRequest::iterator tmpIt = it - 1;
...
void	send_response(int &newSockfd, Response response) {
	// bool	DONE = false;
	// size_t	sent = 0;

	std::cout << KCYN << "FILE SIZE : [" << response.get_length() << "]" << KNRM << std::endl;
	std::cout << KCYN<< "SENT : [" << send(newSockfd, response.get_responseBuffer().c_str(), strlen(response.get_responseBuffer().c_str()), 0) << "]" << std::endl << KNRM;
	// write(newSockfd, sendBuffer.c_str(), sendBuffer.length());
}

void	handle_clients_responses(ClientResponse &write_clients, fd_set &backup_wset)
{
	for(ClientResponse::iterator it = write_clients.begin(); it != write_clients.end(); it++) {
		if (FD_ISSET(it->first.getClientFd(), &backup_wset)) {
			...
			// THIRD INTEGRATION : replace send_simple_response ===>
			std::cout << KCYN << "DBG_SOCKET_FD : [" << it->first.getClientFd() << "]" << KNRM << std::endl;
			send_response(it->first.getClientFd(), it->second);
			// send_simple_response(it->first.getClientFd()); // chunck send
